		...
		public override void PostDrawTiles() {
			...
			if (MiscellaneousTool.showCollisionCircle) {
				...
				for (int i = 0; i < max; i++) {
					...
					if (!showTileCorners)
						Main.spriteBatch.Draw(TextureAssets.MagicPixel.Value, new Rectangle((int)(end.X - Main.screenPosition.X), (int)(end.Y - Main.screenPosition.Y), 1, 1), noCollision ? Color.Green : Color.Red);
